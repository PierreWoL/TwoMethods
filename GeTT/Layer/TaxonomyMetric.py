...
def filter_edges(edges, ground_truth_entity, filter_mode, filter_topk, filter_scores):
    ...
    filter_scores = {child_term.lower(): {parent_term.lower(): filter_scores[child_term][parent_term] for parent_term in
                                          filter_scores[child_term]} for child_term in filter_scores}
    if filter_mode == 'lm_score_ensemble':
        filtered_edges = []
...
def cul_results(model_response, ground_truth_list, mode='edge', new_prompt=False, ChainofLayers=False, filter_mode=None,
                filter_topk=None, filter_scores_list=None):
    '''
    ...
    for i, multi_times_response in enumerate(model_response):
        ...
        for each_time_response in multi_times_response:
            if new_prompt:
                each_edges = [each.strip().strip(';').split(' is a subtopic of ') for each in
                              each_time_response.split(';') if ' is a subtopic of ' in each]
            ...
            if len(each_entities_set) == 0:
                ...
                if not new_prompt:
                    ...
                    if not ChainofLayers and filter_mode is not None and filter_scores_list is not None:
                        each_edges = filter_edges(each_edges, ground_truth_entity, filter_mode, filter_topk,
                                                  filter_scores_list[i])
                ...
                if mode == 'edge':
                    ...
                    precision, recall, f1, [TP, FP, FN] = culculate_entity_coverage(each_entities_set,
                                                                                    ground_truth_entity)
                elif mode == 'ancestor':
                    precision, recall, f1, [TP, FP, FN] = culculate_ancestor_coverage(each_entities_set,
                                                                                      ground_truth_entity, each_edges,
                                                                                      gt_edges)
...
def process_messages(messages):
    ...
    if messages[-1]['role'] != 'assistant':
        ...
        if 'the taxonomy is complete' in messages[-1][
            'content'].lower() and 'check: is the remaining entity list empty?' in messages[-1]['content'].lower():
...
def     eval(taxo_name, taxo_path, model, save_path_model_response, numofExamples=0, file_name='test.json', save=False,
    ...
    if filter_topk is not None and ChainofLayers:
        path = save_path_model_response + taxo_name + '_top' + str(filter_topk) + '/' + model + '/' + str(
            numofExamples) + 'shots/'
    else:
        ...
        if not iteratively:
            for i in range(len(model_response)):
                for j in range(len(model_response[i])):
                    try:
                        model_response[i][j] = remove_empty_lines(
                            model_response[i][j].split('Check: Is the remaining entity list empty?')[-2])
                    except:
    ...
    avg_results_total, avg_results, results, avg_results_merge = cul_results(model_response, ground_truth_edge_list,
                                                                             mode='edge', new_prompt=new_prompt,
    ...
    elif taxo_name in ['wiki_downsample', 'cvd_downsample', 'dblp_downsample', 'dblp_sampled_downsample',
                       'cvd_sampled_downsample']:
        print('[p, r, f]: ', avg_results_total)
        print('[p, r, f]: ', avg_results)
    ...
    avg_results_total, avg_results, results, avg_results_merge = cul_results(model_response, ground_truth_list,
                                                                             mode='ancestor', new_prompt=new_prompt,
                                                                             ChainofLayers=ChainofLayers,
                                                                             filter_mode=filter_mode,
                                                                             filter_topk=filter_topk,
                                                                             filter_scores_list=filter_scores_list)
    ...
    avg_results_total, avg_results, results, avg_results_merge = cul_results(model_response, ground_truth_list,
                                                                             mode='entity', new_prompt=new_prompt,
                                                                             ChainofLayers=ChainofLayers,
                                                                             filter_mode=filter_mode,
                                                                             filter_topk=filter_topk,
                                                                             filter_scores_list=filter_scores_list)
...
def eval_analysis_num_entity(taxo_name, taxo_path, model, save_path_model_response, numofExamples=0,
                             file_name='test.json', save=False, new_prompt=False, ChainofLayers=False,
                             iteratively=False, filter_mode=None, filter_topk=None, filter_scores_list=None):
    assert filter_topk is not None
    for i in ['20', '40', '60', '80', '100', '120', '140', '160']:
        path = save_path_model_response + taxo_name + '_' + i + f'_top{filter_topk}/' + model + '/' + str(
            numofExamples) + 'shots/'
        ground_truth_list = json.load(open(path + 'ground_truth_list.json', 'r'))
        ground_truth_edge_list = []
        for each_gt in ground_truth_list:
        ...
        avg_results_total, avg_results, results, avg_results_merge = cul_results(model_response, ground_truth_edge_list,
                                                                                 mode='edge', new_prompt=new_prompt)
        print('TAXO_NAME:', taxo_name, 'PATH:', save_path_model_response, 'MODEL:', model, 'NUMOFEXAMPLES:',
              numofExamples)
        print('Edge Coverage:')
        print('[p, r, f]: ', avg_results_total)
