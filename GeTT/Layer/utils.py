...
def construct_taxonomy(root, entities, relations):
    ...
    if len(entity_in_relation) != len(entities):
        print("en(entity_in_relation) != len(entities)")
        print(relations)
        ...
        print('Some entities are not in the relations')
    ...
    while True:
        ...
        if add_in_this_iter == False:
            ...
            relations = [relation for relation in relations if
                         relation[0] in entity_level_dict and relation[1] in entity_level_dict]
        ...
        for i in range(times):
            ...
            while response is None:
                if isinstance(client, OpenAI):
                    try:
                        if isinstance(client, OpenAI):
                            response = client.chat.completions.create(
                                ...
                                messages=[
                                    {"role": "system",
                                     "content": 'You are an expert in constructing a taxonomy from a list of concepts.'},
                    ...
                    messages = [
                        ("system", 'You are an expert in constructing a taxonomy from a list of concepts.'),
                        ("user", prompt),
                       """ ("system", "The format of the generated taxonomy is: 1. Parent Concept 1.1 Child Concept. 1.1.1 Grand child Concept."
                                   "Do not change any entity names when building the taxonomy."
                                   "Do not add any extra comments. There should be one and only one root node of the" +
                            "taxonomy. All entities in the entity list must appear in the taxonomy and don't" +
                ...
                if check:
                    ...
                    if new_prompt:
                        each_edges = [each.strip().strip(';').split(' is a subtopic of ') for each in
                                      response_text.split(';') if ' is a subtopic of ' in each]
