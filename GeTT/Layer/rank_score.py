...
def get_terms(taxo_name, fold):
    terms = []
    ##I changed here
    path = os.getcwd() + '/Layer/dataset/processed/' + taxo_name + f'/{fold}/test.json'
    print(path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--taxo_name', type=str, default='GDS')
    parser.add_argument('--fold', type=str, default='gpt3.5')
    parser.add_argument('--save_path', type=str, default=f'./Layer/filter/')
    ...
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    save_path = save_path + taxo_name + '/' + fold + "/"
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    print(f"save path {save_path}")

    terms_list = get_terms(taxo_name, fold)
    fw = open(save_path + 'scores.json', 'a')
    ...
    for terms in tqdm(terms_list):
        ...
        print(f"{terms} running time of filter scoring: {elapsed_time:.4f} s")
