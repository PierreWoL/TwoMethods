        ...
        if numofExamples == 0:
            if mode == 0:
                for i in range(5):
                    ...
                    prompt_list.append(prompt_temp.replace('<root>', root) + "Entity List: " + str(
                        sample_entities) + "\n" + "Taxonomy:\n")
            elif mode == 1:
...
def pharse_model_response_to_json(model_response_path, save_path, taxo_name, file_name='test.json'):
    ...
    with open(save_path + file_name, 'w') as f:
        for taxo in taxos:
            ...
            for root in roots:
                dict = {'root': root, 'relation_list': subtree[root]['relation_list'],
                        'entity_list': subtree[root]['entity_list']}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=int, default=1)  # 0
